...
Eolian is a Discord music bot built with TypeScript and Vite. The Node bot/server build is emitted
as CommonJS at `dist/bundle.js`, and the browser build is emitted to `dist/public`.
...
Modules use the `@eolian/*` path alias rooted at `src/` (defined in `src/tsconfig.json` and
`vite.node.config.mts`). Each module exports through `index.ts` and defines types in `@types.ts`.
...
- `__COMMIT_DATE__` - Injected by the Node Vite build from git log (shown in bot status)
